...
function doors(n){
    let doorArr = new Array(n).fill('closed')
    for (let i = 1; i <= n; i++) {
       for (let j = i - 1; j < n; j+= i) {
         doorArr[j] == 'closed'? doorArr[j]= 'open' :  doorArr[j] = 'closed'
       }  
    }
    return doorArr.filter(el=> el == 'open').length
  }
  
  //TOO SLOW
  // doorArr = doorArr.map((el, index)=> {
  //       if ((index+1) % i == 0) {
  //         return el == 'closed'? 'open' : 'closed'
  //       }
  //       return el
