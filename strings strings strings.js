...
// [true,false].toString() === "[true,f

Array.prototype.toString = function () {
  
    function stringer(el) {
      let result = '['
      for (let i = 0; i < el.length; i++) {
        
        if (typeof el[i] == 'string') {
          result += i>0? `,'${el[i]}'` : `'${el[i]}'`
        } else if (typeof el[i]=='number') {
          result += i >0? `,${el[i].toString()}` : `${el[i].toString()}`
        } else if (typeof el[i]=='boolean') {
          if (el[i]) {
            result += i >0? `,true` : `true`
          } else {
            result += i >0? `,false` : `false`
          }
          
        } else {
          result += i > 0? `,${stringer(el[i])}` : `${stringer(el[i])}`
        }
      }
      result += ']'
      return result
