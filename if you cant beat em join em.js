...
function cantBeatSoJoin(numbers) {
    ...
    console.log("The strengthValue of the strongest gangs is: " + strengthValue)
    
    let sortedStrengthValue = [...strengthValue]
    sortedStrengthValue = sortedStrengthValue.sort((a, b)=> b-a)
    console.log("the strength values sorted " + sortedStrengthValue)
    
    let result = []
    for (let i = 0; i<numbers.length; i++) {
      result.push(numbers[strengthValue.indexOf(sortedStrengthValue[i])])
    }
    console.log(result)
    return result.flat()
  }