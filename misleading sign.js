...
function gaslighting(shirtWord,yourWord,friendsLetters) {
    //find difference between shirtWord and yourWord. 
    //If friendsLetters include unique letters from shirtWord --false
    //or if friendsLetters include trick letters from yourWord --false
    //if friend doesnt know if they are tricked --false
    //if you dont try to trick friend-- false
    let trickyLetters= []
    for (let i = 0; i<shirtWord.length; i++) {
      if(shirtWord[i]!=yourWord[i]) {
        trickyLetters.push(shirtWord[i], yourWord[i])
      }
    }
    for (let x = 0; x < friendsLetters.length; x++) {
      if (trickyLetters.includes(friendsLetters[x])) {
        return true
      }
    }
    return false
