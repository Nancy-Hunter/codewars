...
function findWord(numLet, maxSsw) {
    let filteredWordList = wordList.filter(x=> x.length ==numLet)
    let sswWordList = filteredWordList.map(x=>x.split('').reduce((acc, current, index)=> acc + current.charCodeAt(0),0))
    let targetIndex = sswWordList.lastIndexOf(maxSsw)
    while (maxSsw>0 && targetIndex<0) {
      maxSsw--
      targetIndex = sswWordList.lastIndexOf(maxSsw)
      if(maxSsw==0){
        return null
      }
      }
    return filteredWordList[targetIndex]
}