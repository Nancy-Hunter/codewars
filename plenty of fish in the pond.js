...
function fish(shoal) {
    ...
    let step = 4
    let count = 0
    shoal = shoal.split('').filter(el=> el != 0).map(el=> +el)
    while (shoal.filter(el=> el<=size).length) {
      let tastiest = step - count < eatable? step- count: eatable
      while (!shoal.includes(tastiest) && tastiest>0) tastiest--
      if (shoal.includes(tastiest)) {
        count += tastiest
        shoal.splice(shoal.indexOf(tastiest), 1)
        if (count >= step) {
          size++
          eatable++ 
          step+= 4
