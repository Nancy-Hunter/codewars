...
function checkDNA (seq1, seq2) {
  ...
  let compare, originStrand
  if(seq1.length > seq2.length) {
    compare = seq2.split('').reverse() 
    originStrand = seq1
  } else {
    compare = seq1.split('').reverse() 
    originStrand = seq2
  }
  compare = compare.map(x=> dna[x]).join('')
  return originStrand.indexOf(compare)> -1? true : false
}
