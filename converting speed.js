...
function calculateSpeed(distance, time) {
    let distanceInMeters = distance.split('').includes('k')?  distance.split('').slice(0,-2).join('') * 1000:distance.split('').slice(0,-1).join('')
    let timeInSeconds = time.split('').includes('m')?  time.split('').slice(0,-3).join('') * 60: time.split('').slice(0,-1).join('')
    let mPerS = distanceInMeters/timeInSeconds
    let mph = Math.round(mPerS * 2.23694)
    return mph + 'mph'
  }