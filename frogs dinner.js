...
function frogContest(n) {
    let chrisTotal = (n*(n+1))/2
    let tom = Math.floor(chrisTotal/2)
    let tomTotal = (tom*(tom+1))/2
    let cat = chrisTotal + tomTotal
    let catTotal = (cat*(cat+1)/2)
    return `Chris ate ${chrisTotal} flies, Tom ate ${tomTotal} flies and Cat ate ${catTotal} flies`
  }